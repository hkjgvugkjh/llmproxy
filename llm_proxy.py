...
def get_logger(ymd: str) -> logging.Logger:
    """Return a logger that writes to {ymd}.log (one file per calendar day)."""
    fh = logging.FileHandler(
        os.path.join(log_dir, f"{ymd}.log"), encoding="utf-8"
    )
...
async def list_models(request: Request):
    verify_auth(request.headers.get("Authorization"))
    data = [{"id": m, "object": "model", "created": int(datetime.now().timestamp()), "owned_by": "llmproxy"} for m in models]
...
async def proxy(request: Request, path: str):
    ...
    log.info(">>> INPUT  MODEL=%(model)s  BODY=%(body)s", {"name":model_name , "model": model, "body": input_text , })
    ...
    async with httpx.AsyncClient(timeout=300) as client:
        try:
            ...
            if "text/event-stream" in content_type:
                ...
                log.info(
                    "<<< OUTPUT MODEL=%(model)s  STATUS=%(status)d  BODY=%(body)s",
            ...
            log.error("<<< ERROR  MODEL=%(model)s  REASON=%(reason)s", {"model": model_name, "reason": str(e)})
